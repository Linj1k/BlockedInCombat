...
public class PlayerManager {
    ...
    private final HashMap<UUID, PlayerData> PlayersStats = new HashMap<>();
    ...
    public void loadPlayerData(Player player){
        ...
        if(!main.getPlayerManager().getPlayersStats().containsKey(uuid)){
            ...
            if(f.exists() && !f.isDirectory()) {
                ...
                main.getLogger().info(Main.getPrefix(true)+" Loading of "+player.getName()+"("+ uuid +")'s data");
            } else {
                main.getPlayerManager().getPlayersStats().put(uuid, new PlayerData(0,0,0,0,0,0));
                main.getLogger().info(Main.getPrefix(true)+" Creation of the data of "+player.getName()+"("+ uuid +")");
            }
        }
    }
    ...
    public static boolean hasMovedOneBlock(Location from, Location to){
        final Location bFrom = Objects.requireNonNull(from.getWorld()).getBlockAt(from).getLocation();
        final Location bTo = Objects.requireNonNull(to.getWorld()).getBlockAt(to).getLocation();
        return bFrom != bTo;
    }
