...
public class BuildArena extends BukkitRunnable {
    ...
    @Override
    public void run() {
        if(main.getTeamsManager().getValidTeams().size() == 1 || !main.getTeamsManager().CheckForStart() || Bukkit.getOnlinePlayers().size() <= 1) {
        ...
        if(timer == 5 || timer == 3 || timer == 2 || timer == 1) {
            Bukkit.broadcastMessage(main.getPrefix()+ Lang.GAMESTATEÙGAME_START.get().replace("{time}", String.valueOf(timer)));
        }
